...
Betrachte dann das Polynom
\[
  A(x) = a_nx^n + \dots + a_1x + a_0
\]
mit $\deg(A)=n$ und
\[
  B(x) = b_kx^k + \dots + b_1x + b_0
\]
mit $\deg(B)=k$ wobei $k < n$. Wähle dann das Polynom
\[
  P(x) = \frac{a_n}{b_k}x^{n-k}
\]
Nun gilt für
\[
  A'(x) = A(x) - B(x)P(x)
\]
$\deg(A') < n$ (einfaches nachrechnen).
Somit kann aber auch auf $A'$ die Induktionsvorraussetzung angewandt werden.
\[
  A'(x) = Q'(x)B(x) + R'(x)
\]
Damit ist
\[
  A(x) = B(x)P(x) - A'(x) = B(x)P(x) - Q'(x)B(x) + R'(x) = B(x)(P(x) - Q'(x)) + R'(x) 
\]
von der geforderten Form.
\paragraph{Eindeutigkeit}
Seien $A(x) = Q(x)B(x) + R(x)$ und $A(x) = Q'(x)B(x) + R'(x)$. Setze
\[
  Q(x)B(x) + R(x) = Q'(x)B(x) + R'(x) \iff B(x)(Q(x) - Q'(x)) = R'(x) - R(x) 
\]
Da angenommen werden kann dass $\deg(R') \neq \deg(R)$ (da sonst die Behauptung
schon erfüllt wäre). Betrachte dass $R'(x) - R(x)$ ein Vielfaches von $B(x)$
sein muss. Laut Voraussetzung gilt $\deg(R'-R) \leq \max(\deg(R),\deg(R')) <
\deg(B)$, dann kann aber $R'-R$ kein Vielfaches von $B$ sein. Das ist ein
Widerspruch.
\hfill $\square$

\paragraph{Lemma 6.45}
Ein Ring $R$ mit Division mit Rest ist ein Hauptidealring.
\paragraph{Beweis}
Sei $I \subset R$ Ideal mit $I \neq (0)$, da dieser Fall trivial ist. Definiere
zunächst für $0 \neq b \in I$
\[
  \phi(b) = \min\{\phi(x) ~|~ x \in I, x \neq 0\}
\]
Dann gilt $I = (b)$. Tatsächlich kann man für jedes Element $a \in I$ schreiben
$a = qb + r$ mit $\phi(r) < \phi(b)$ wobei $r \in I$ und $\phi(b)$ minimal ist.
Da $r \in I$ gilt $r = 0$ und somit $a \in (b)$.
Daraus folgt die Behauptung. \hfill $\square$
...
\end{enumerate}
\paragraph{Beweis}
Angenommen es gibt $d \neq d'$ mit $d=d'=\gcd(a,b)$.
Wegen Definition des $\gcd$ gilt auch $d|d'$ und $d'|d$. O.B.d.A ist $d > d'$
dann ist $d|d'$ ein Widerspruch.

\paragraph{Lemma 6.47}
Sei $R$ ein Hauptidealbereich $\Rightarrow \forall a,b \in R: \exists ~ \gcd(a,b)$
\paragraph{Beweis}
Da $R$ Hauptidealbereich betrachte das Ideal $I = (a,b) = (d)$. Nun ist zu
zeigen dass $\gcd(a,b)=d$. Da $a \in I = (d)$ und $b \in I = (d)$ folgt schon
dass $d|a \wedge d|b$. Angenommen es gibt noch ein $d' \in (a,b)$ mit $d'|a
\wedge d'|b$. Da gilt $d = ax + by$, gilt $d'|d$. Somit ist $d$ von den
...
\[
  d = \gcd(a,b) \Rightarrow d = a\alpha + b\beta, ~\text{mit}~ \alpha,\beta \in R
\]
...
\paragraph{Beweis}
Sei $I_0 \subset I_1 \subset \dots$ so eine aufsteigende Kette von Idealen.
Betrachte $I=\bigcup_{n=0}^\infty I_n$ dann gilt $I = (a)$ wegen Hauptidealring.
Somit gilt aber auch $\exists n \in \N: a \in I_n$. Daraus folgt aber auch $I =
I_k$ f.a. $k \geq n$. Damit ist diese Kette stationär.
\hfill $\square$ 
...
f.a. $i = 1, \dots, r$.
\paragraph{Beweis}
Da $p_1|q_1 \dots q_s \Rightarrow \exists n \in \{1,\dots,s\}: p_1|q_n $. Nach
Umnummerierung, kann angenommen werden $p_1|q_1 \Rightarrow p_1 \cdot x =q_1$.
mit $x \in R^*$. Daraus folgt $p_2 \cdots p_r = xq_2 \cdots q_s$. Diesen Prozess
kann man induktiv fortsetzen, sodass am Ende alle $p$ und $q$ verschwinden.
Damit müssen es genau gleich viele gewesen sein.
\hfill $\square$
...
\begin{enumerate}{}
\item[$\Rightarrow$)] Sei $q \in R$ irreduzibel. Wähle dann $x,y \in R: q|xy$.
  Seien $x=x_1 \dots x_n$ und $y=y_1 \dots y_n$ die eindeutigen Zerlegungen in
  irreduzible Elemente. Wegen der Endeutigkeit muss gelten $q|x_i$ oder $q|y_j$.
  daraus folgt aber auch $q|x$ oder $q|y$. Damit ist $q$ ein Primelement.
  Reihenfolge und Eindeutigkeit folgen aus vorherigem Lemma.
\item[$\Leftarrow$)] Aus einem Lemma folgt wenn Element prim dann irreduzibel.
  Eindeutigkeit folgt aus vorherigem Lemma.
\end{enumerate}
\hfill $\square$
...
\paragraph{Beweis}
\begin{enumerate}{}
  \item[$\Rightarrow$)] O.B.d.A $b\neq 0$ Dann gilt
\[
  (a) = (b) \Rightarrow a \in (b) \Rightarrow bx = a
\]
\[
  (a) = (b) \Rightarrow b \in (a) \Rightarrow ay = b
\]
$\Rightarrow b= bxy \Leftrightarrow 0 = bxy - b \Leftrightarrow 0 = b(xy - 1)$.
Damit muss aber $xy = 1$ und da $1$ Einheit folgt $x \in R^*$ oder $y \in R^*$.
\item[$\Leftarrow$)] klar.
\end{enumerate}
\hfill $\square$
...
\]
\paragraph{Beweis}
O.b.d.A $p \not| a$. Da sonst fertig. Zu zeigen ist $p|b$
\[
  p \not| a \implies \exists x,y \in R: px + ay = 1 \implies b(px + ax) = b 
\]
Da $p$ linke Seite teilt, teilt $p$ auch rechte Seite.
\hfill $\square$
...
\]
\paragraph{Beweis}
Sei $a=bc$ Faktorisierung. Da $(a)$ prim folgt o.B.d.A $b \in (a)$. Damit
\[
  b = ax = bcx \Rightarrow cx = 1.
\]
Somit ist $c$ invertierbar und damit $a$ irreduzibel.
\hfill $\square$
...
\paragraph{Beweis}
Sei $a \in R\backslash (R \cup 0)$
\begin{enumerate}{}
  \item $a$ hat einen irreduziblen Teiler. Falls $a$ irreduzibel dann fertig.
    Falls nicht, dann ist $a$ reduzibel. Dann zerlege $a$ in Nichteinheiten $a =
    a_1x$. Falls nun $a_1$ irreduzibel dann fertig. Wenn nicht dann wiederhole
    den letzten Schritt. Auf diese Weise erhält man eine aufsteigende Kette von
    Idealn
    \[
      (a) \subset (a_1) \subset (a_2) \subset \dots
    \]
    Diese Kette wird stationär da $R$ noethersch ist. Somit gibt es ein $(a_N) =
    (a_i)$ f.a. $i \geq N$. Damit ist $a_N$ irreduzibel.
  \item $a$ lässt sich als Produkt von irreduziblen Elementen schreiben. Nach 1.
    finden wir ein irreduzibles $q_1$ sodass $q_1|a$ somit $a=q_1a_1$ mit $a_1$
    kann man nun das gleiche durchführen. So erhält man eine eindeutige
    Zerlegung $a = q_1\dots q_n$. Oder eine nicht stationäre Kette von Idealen
    $(a) \subset (q_1) \subset (q_2) \subset \dots $. Das ist aber nicht möglich
    da $R$ noethersch ist.
  \item Die Eindeutigkeit folgt aus dem vorherigen Lemma.
\end{enumerate}
\hfill $\square$
...
\paragraph{Theorem 6.74} \textit{(Satz von Gauß)}
...
(insbesondere kann $R$ ein Körper sein). \\
\textit{Beweis} \\
\[
...
\]
\paragraph{Beweis}
Für konstante Polynome ist die Behauptung klar. Sei nun o.B.d.A. $f,g \in R[x]$
(mit Nenner multiplizieren) mit $v_p(f)=0=v_p(g)$ (Koeffizienten durch $\gcd$ der
Koeffizienten teilen). Es ist nun zu zeigen dass $v_p(fg)=0$. Betrachte dazu den
Homomorphismus
\[
  \phi: R[x] \longrightarrow (R/(p))[x],
\]
in diesem verschwinden ($\ker(\phi)$) alle Polynome die nur Koeffizienten haben, die durch $p$
teilbar sind. Betrachte darin
\[
  \phi(g)\phi(g) = \phi(fg) \neq 0
\]
Also sind auch bei $fg$ nicht alle Koeffizienten durch $p$ teilbar. Damit folgt
$v_p(fg)=0$.
\hfill $\square$
...
\paragraph{Beweis}
\begin{enumerate}
\item $v_p(h)=0 \Rightarrow v_p(f) \leq 0, v_p(g) \leq 0 \Rightarrow
  v_p(f)=v_p(g)=0 \Rightarrow f,g \in R[x]$.
\item $v_p(h) \geq 0, v_p(f)=0 \Rightarrow v_p(g) \geq 0 \Rightarrow g \in R[x]$.
\end{enumerate}
  \hfill $\square$
...
\begin{enumerate}{}
\item klar.
\item Betrachte $K_1,K_2$ Körper mit $\phi: K_1 \to K_2$ dann muss auch in $K_2$
  gelten $0 \neq 1$ ein Homomorphismus von Körpern ist ein Ringhomomorphismus
  und somit muss $\ker(\phi)$ ein Ideal sein. Somit kann $\ker(\phi) = K$ nicht
  gelten da dann $0=1$ in $K_2$ wäre. Somit $\ker(\phi)=(0)$ und damit $\phi$ injektiv.
\end{enumerate}
\hfill $\square$
...
\end{enumerate}
\paragraph{Beweis}
$\tau: \Z \to P,~~n \mapsto n1$ ist Ringhomomorphismus. Damit liegen alle
Vielfachen von $n1$ in $P$.
\begin{enumerate}{}
\item Falls $char(K)=0$ gilt $\ker(\tau)=0$. Damit ist $\Z \simeq \tau(\Z)$.
  Damit folgt
  \[
    \Q = Q(\Z) \subset P.
  \]
  Da aber nach Def. $P$ in allen Teilkörpern liegt folgt $\Q=P$
\item Hier ist $\ker(\tau)=(p)$. Nach Homomorphiesatz folgt
  \[
    \tau(\Z) \simeq \Z/p\Z = \F_p.
  \]
  $\F_p$ ist Körper somit ist $\F_p \subset P$. Wie in Punkt 1 folgt nun die Gleichheit. 
\end{enumerate}
\hfill $\square$

\paragraph{Korollar 7.11}
Sei $\phi: L\to K$ ein Homomorphismus von Körpern. Dann gilt
\begin{enumerate}{}
\item $char(L)=char(K)$
...
\paragraph{Beweis}
\begin{enumerate}{}
\item Jeder Homomorphismus von Körpern ist injektiv damit gilt $\phi(e_L)=e_K$.
  Somit
  \[
    ub{\phi(e_L) + \dots \phi(e_L)}{p-mal} = 0 \Rightarrow \ub{e_K + \dots e_K}{p-mal} = 0
  \]
  Damit haben $K$ und $L$ die gleiche Charakteristik.
...
\[
  [L:F]=[L:K]\cdot[K:F]
\]
Genauer: Sind $(k_i)_{i\in I}$ eine Basis von $L/K$ und $(f_j)_{j\in J}$ eine
Basis von $K/F$, so ist
\[
  (k_j,f_i)_{(j,i) \in J \times I}
\]
eine Basis von $L/F$.
\paragraph{Beweis}
$a \in L$ kann dargestellt werden als $a = \sum_{i \in I}\lambda_ik_i$ wobei
$k_i = \sum_{j \in J}\mu_jf_j$. Also
\[
  a = \sum_{(i,j) \in I \times J}\lambda_i\mu_jf_{ij}
\]
Somit ist $  (k_j,f_i)_{(j,i) \in J \times I}$ ein $F$-Erzeugendensystem von
$(L,+)$. Aus
\[
  \sum_{(i,j) \in I \times J}\lambda_i\mu_jf_{ij} = 0 ~\text{d.h.}~
  \sum_{i \in I}\lambda_i\left(\sum_{j \in J} \mu_jf_{j} \right) =
  0,~\text{folgt}~\sum_{j\in J}
\]
\dots Muss verbessert werden. 
...
\paragraph{Beweis}
Sei $n=[L:K]$ damit sind die $n+1$ Elemente $1+a+a^2+\dots+a^n \in L/K$ linear
abhängig. Somit ist $\lambda_01 + \lambda_1a_1 + \dots + \lambda_na^n = 0$ für
gewisse $\lambda_0,\dots,\lambda_n \in K$. Folglich ist $a$ Nullstelle. Damit
...
\paragraph{Beweis}
\begin{enumerate}{}
\item $K[x] \to K[\alpha]$ ist surjektiv. Daher nach Homomorphiesatz $K[\alpha]
  \simeq K[x]/(m_a,K)$. Da $K[x]$ Hauptidealring, erzeugt das irreduzibele $m_a,K$
  Polynom ein maximales Ideal $(m_a,K)$, sodass $K[x]/(m_a,K)$ ein Körper ist.
  Damit gilt $K[x]=K(x)$.
\item Bezeichne $p(x)=m_{a,K}$. $(1,a,^2,\dots,a^{n-1})$ stellt eine Basis von
  $K(a)/K$ dar. Denn
  \[
    \lambda_01 + \lambda_1a + \dots + \lambda_{n-1}a^{n-1}=0,~\lambda_i \in K ~~ n=\deg(m_{a,K})
  \]
  Dann ist $Q(a)=0$ und $\deg(Q) < n$ für $Q$ wie oben nur $x$ statt $a$. Nun
  gilt aber $m_{a,K}|Q(a)$ und damit muss $Q(a)$ schon $0$ sein. Damit sind
  $(1,a,^2,\dots,a^{n-1})$ linear unabhängig. Erzeugendensystem.. 
\end{enumerate}
...
L/K$ ist endlich.
\paragraph{Beweis}
Man kann $K(a_1,\dots,a_n)$ schreiben als $K(a_1)(a_2)\dots(a_n)$ mit dem
Gradsatz folgt die Endlichkeit.
\hfill $\square$
...
Ein Körper $K \subset L$ heißt Zerfällungskörper von $f(x)$, wenn $f(x)$ lässt
...
\item Ist $\Q(\sqrt[3]{2}) \subset \R$ Zerfällungskörper von $x^3-2$? nein. Es
  fehlen Nullstellen.
\end{enumerate}
